...
from ops import se3, invmat
from .pointnet import PointNet
from .pooling import Pooling


class PointNetLK(nn.Module):
    def __init__(self, feature_model=PointNet(), delta=1.0e-2, learn_delta=False, xtol=1.0e-7, p0_zero_mean=True,
    ...
    def approx_Jic(self, template, template_features, dt):
        ...
        f = self.pooling(self.feature_model(p.view(-1, num_points, 3))).view(batch_size, 6, -1).transpose(1,
                                                                                                          2)  # [B, K, 6]
    ...
    def handle_batchNorm(self, template, source):
        ...
        if training:
            ...
            template_features, source_features = self.pooling(self.feature_model(template)), self.pooling(
                self.feature_model(source))
