...
class RPMNet(nn.Module):
    ...
    def forward(self, template, source, max_iterations: int = 1):
        ...
        for i in range(max_iterations):
            weighted_template = self.spam(xyz_template, norm_template, xyz_source_t,
                                          norm_source_t)  # Finding better correspondences after each iteration.
            ...
            xyz_source_t, norm_source_t = se3_transform(transform.detach(), xyz_source,
                                                        norm_source)  # Apply transformation to original source.
        ...
        est_T = convert2transformation(transforms[max_iterations - 1][:, :3, :3],
                                       transforms[max_iterations - 1][:, :3, 3])
